...
public class CustomView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // Рисуем окно на доме
        paint.setColor(Color.rgb(135, 206, 235)); // Голубой цвет окна
        ...
        // Рисуем ёлку
        paint.setColor(Color.GREEN); // Цвет ёлки
        Path tree = new Path();
        tree.moveTo(650, 400);
        tree.lineTo(700, 300);
        tree.lineTo(750, 400);
        ...
        canvas.drawRect(690, 500, 710, 600, paint);

        // Рисуем солнце
        paint.setColor(Color.YELLOW);
        canvas.drawCircle(800, 100, 50, paint);

        // Рисуем лучи солнца
        for (int i = 0; i < 360; i += 45) {
            float startX = (float) (800 + 50 * Math.cos(Math.toRadians(i)));
            float startY = (float) (100 + 50 * Math.sin(Math.toRadians(i)));
            float endX = (float) (800 + 100 * Math.cos(Math.toRadians(i)));
